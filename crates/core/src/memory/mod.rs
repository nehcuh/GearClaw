...
impl MemoryManager {
    ...
    pub async fn sync(&self) -> Result<(), GearClawError> {
        ...
        let pattern_str = pattern.to_str().ok_or_else(|| GearClawError::ConfigParseError("Invalid workspace path".to_string()))?;
        
        let mut files_to_process = Vec::new();
        let mut current_paths = std::collections::HashSet::new();

        for entry in glob(pattern_str).map_err(|e| GearClawError::IoError(std::io::Error::new(std::io::ErrorKind::Other, e)))? {
            match entry {
                Ok(path) => {
                    if path.is_file() {
                        ...
                        let mtime = metadata.modified()?.duration_since(std::time::UNIX_EPOCH).unwrap().as_secs() as i64;
        ...
        // Remove deleted files
        {
            ...
            let stored_paths: Vec<String> = stmt.query_map([], |row| row.get(0))?
        ...
        for (abs_path, rel_path, mtime, size) in files_to_process {
            ...
            for (i, chunk_text) in chunks.iter().enumerate() {
                ...
                let chunk_id = format!("{:x}", Sha256::digest(format!("{}:{}:{}", rel_path, i, chunk_text).as_bytes()));
    ...
    pub async fn search(&self, query: &str, limit: usize) -> Result<Vec<SearchResult>, GearClawError> {
        ...
        let mut scored: Vec<SearchResult> = chunks.into_iter().map(|(_id, path, text, embedding, start_line)| {
