...
MCP 已全量启用（`BUILD_MCP_CAPABILITY = Enabled`）。扩展方式：

### 7.1 添加自定义工具到 Agent

1. 在 `crates/tools/src/lib.rs` 的 `available_tools()` 中添加 `ToolSpec`
2. 在 `crates/core/src/agent.rs` 的 `execute_tool_call` 中添加匹配分支

### 7.2 添加 MCP 服务器

1. 通过 CLI：`gearclaw mcp install <id>`
2. 手动编辑 `config.toml` 的 `[mcp.servers.*]` 段
3. 通过 Agent 对话：请 Agent 调用 `mcp_install_server` 工具

### 7.3 扩展内置注册表

1. 在 `crates/mcp/src/registry.rs` 的 `REGISTRY` 中添加 `RegistryEntry`
2. 提供 `id`、`name`、`description`、`install_method`、`package`、`command`、`default_args`、`tags`
